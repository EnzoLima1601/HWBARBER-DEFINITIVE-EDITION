...
function clicarBotao() {
  ...
  var escolherServico1 = document.getElementById("clickIdServico1");
  if(escolherServico1){
    escolherServico1.addEventListener("click", function(){
      window.location.href="../Pagamento/carrinhocutpoint.html"
    })
  } 
...
function Pagamento() {
  ...
  if (pagamentoPix) {
    pagamentoPix.addEventListener("click", function () {
      window.location.href = "../Pagamento/TelaPix.html";
      //setTimeout(function () {
  ...
  var pagamentoCut = document.getElementById("PagamentoCut");
  if (pagamentoCut) {
    pagamentoCut.addEventListener("click", function() {
      // Obter o valor atual dos cutpoints
      let numero = parseInt(localStorage.getItem("cutpoints")) || 0;
  
      // Verificar se há cutpoints suficientes para a compra
      if (numero >= 100) {
        numero -= 100; // Subtrair os cutpoints
        localStorage.setItem("cutpoints", numero);
  
        // Atualizar o valor exibido de cutpoints na página
        const cutpointsSpan = document.getElementById("cutpointsValor");
        if (cutpointsSpan) {
        ...
        window.location.href = "../Pagamento/FinalizadoCut.html";
      } else {
        // Caso não tenha cutpoints suficientes, exibir uma mensagem (opcional)
        alert("Você não possui cutpoints suficientes para essa compra!");
      }
    });
  }
}
...
function redirecionarParaProximaPagina() {
  setTimeout(function () {
    ...
    window.location.href = "../Pagamento/Finalizado.html"; // Substitua com a URL da próxima página
...
function telaCutpoint(){
  var Trocatela = document.getElementById("SetaCutpoint");
  if(Trocatela) {
    Trocatela.addEventListener("click", function(){
      window.location.href = "../Servicos/servicocutpoint.html";
    })
  }
  var TrocaTelaservico = document.getElementById("SetaCutpoint1");
  if(TrocaTelaservico) {
    TrocaTelaservico.addEventListener("click", function(){
      window.location.href = "../Servicos/servico1.html";
    });
  }
}
